...
import { guardToolExecutionDuringLock, waitForOrdinaryChatGateIfNeeded } from "../../src/runtime/send-entry-gate.js";
...
test(
  "explicit absolute-path plugin loading initializes hooks for a temp project and writes operator-visible side effects",
  async () => {
    const tempDirectory = await mkdtemp(join(tmpdir(), "opencode-context-compression-e2e-load-"));
    ...
    try {
      ...
      const pluginModule = (await import(pathToFileURL(PLUGIN_ENTRY).href)) as {
        ...
        }) => Promise<Record<string, (...args: unknown[]) => Promise<void> | void>>;
      };
      ...
      assert.match(savedConfig, /\/root\/_\/opencode\/opencode-context-compression\/src\/index\.ts/u);
      assert.ok(
        observations.some((observation) => observation.seam === "tool.execute.before"),
      ...
      assert.ok(
        observations.some((observation) =>
          observation.identityFields.some(
            (field) => field.path === "pluginInit.worktree" && field.value === tempDirectory,
          ),
        ),
...
test("keep route persists committed sidecar state and projects deterministically across reruns", async () => {
  await withTempEnvironment(async ({ projectDirectory, lockDirectory, store, clock }) => {
    const canonicalMessages = [
      ...
      createEnvelope(createMessage({ id: "user-2", role: "user", created: 5 }), [createTextPart("user-2", "next")]),
    ];
    ...
    assert.deepEqual(
      querySqlite(store.databasePath, "SELECT status FROM compaction_batches ORDER BY batch_id;"),
      ["succeeded"],
    );
  });
});
...
async function withTempEnvironment(
  ...
  const projectDirectory = await mkdtemp(join(tmpdir(), "opencode-context-compression-e2e-keep-"));
...
async function runFrozenBatch(store: SqliteSessionStateStore, lockDirectory: string, batchID: string) {
  const { freezeCurrentCompactionBatch } = await import("../../src/marks/batch-freeze.js");
