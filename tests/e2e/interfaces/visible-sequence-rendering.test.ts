...
test(
  ...
  async (t) => {
    ...
    assert.match(expectedIds.assistant.assignedVisibleId, /^compressible_000002_[0-9A-Za-z]{8}$/u);
    assert.match(expectedIds.tool.assignedVisibleId, /^compressible_000003_[0-9A-Za-z]{8}$/u);
    ...
    assert.match(
      ...
      /^reminder_000002_[0-9A-Za-z]{8}$/u,
    );
