...
import { acquireSessionFileLock, releaseSessionFileLock } from "../../src/runtime/file-lock.js";
import {
...
function seedMarkSet(store: SqliteSessionStateStore, markIDs: readonly string[]): void {
  ...
  markIDs.forEach((markID, index) => {
    persistMark({
      ...
      route: "keep",
      sourceMessages: index % 2 === 0 ? [{ hostMessageID: "src-1" }] : [{ hostMessageID: "src-2" }],
    });
  });
}
...
async function withTempEnvironment(
  ...
  const pluginDirectory = await mkdtemp(join(tmpdir(), "opencode-context-compression-send-entry-"));
