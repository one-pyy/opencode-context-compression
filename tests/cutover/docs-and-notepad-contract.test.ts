...
const LIVE_GUIDE_PATH = "docs/live-verification-with-mitmproxy-and-debug-log.zh.md";
const INDEX_PATH = ".sisyphus/notepads/INDEX.md";
const DECISION_PATH = ".sisyphus/notepads/decisions/2026-04-03_final-repo-owned-operator-contract.md";
const TUTORIAL_PATH = ".sisyphus/notepads/tutorials/2026-04-03_repo-owned-operator-and-live-verification-workflow.md";
...
test("operator docs advertise only the final repo-owned contract", async () => {
  ...
  assert.match(liveGuide, /tests\/cutover\/runtime-config-precedence\.test\.ts/u);
  assert.match(liveGuide, /tests\/cutover\/legacy-independence\.test\.ts/u);
  assert.match(liveGuide, /tests\/cutover\/docs-and-notepad-contract\.test\.ts/u);
  assert.match(liveGuide, /tests\/e2e\/plugin-loading-and-compaction\.test\.ts/u);
  ...
  assert.match(liveGuide, /真实会话里的 live verification 目前适合确认“插件确实加载了/u);
});

test("target repo notepad records the final operator contract and workflow", async () => {
  await Promise.all([access(new URL(`../../${DECISION_PATH}`, import.meta.url)), access(new URL(`../../${TUTORIAL_PATH}`, import.meta.url))]);
  ...
  assert.match(index, /\[decisions\/2026-04-03_final-repo-owned-operator-contract\]/u);
  assert.match(index, /\[tutorials\/2026-04-03_repo-owned-operator-and-live-verification-workflow\]/u);
  ...
  assert.match(tutorial, /^## repo-owned-operator-and-live-verification-workflow$/mu);
  ...
  assert.match(tutorial, /`docs\/live-verification-with-mitmproxy-and-debug-log\.zh\.md`/u);
  assert.match(tutorial, /`tests\/cutover\/docs-and-notepad-contract\.test\.ts`/u);
});
