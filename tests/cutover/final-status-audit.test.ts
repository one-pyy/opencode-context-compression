...
import { listRepoFiles, listVisibleRepoFiles, listWorkspaceFiles, readWorkspaceFile } from "./cutover-test-helpers.js";

const STALE_PLAN_PATH = ".sisyphus/plans/opencode-context-compression-plugin.md";
const CUTOVER_PLAN_PATH = ".sisyphus/plans/opencode-context-compression-full-cutover.md";
...
test("final evidence pack enumerates the current executable cutover proof surface", async () => {
  const [workspaceEvidenceFiles, repoCutoverTests, evidencePack] = await Promise.all([
  ...
  for (const testPath of CURRENT_CUTOVER_TESTS) {
    assert.ok(visibleCutoverTests.includes(testPath), `expected current cutover test '${testPath}' to exist`);
  ...
  assert.match(evidencePack, /public tool proof comes from the current `compression_mark` cutover test/u);
  assert.match(evidencePack, /config precedence proof comes from the repo-owned runtime config audit/u);
  assert.match(evidencePack, /scheduler live path proof comes from the `chat\.params` scheduler audit/u);
  assert.match(evidencePack, /legacy independence proof comes from the absence and fixture audit/u);
  assert.match(evidencePack, /docs and notepad proof come from the Task 8 contract audit/u);
});
...
test("status audit derives current cutover truth from evidence instead of inherited checkbox state", async () => {
  ...
  for (const uncheckedLinePrefix of [
    ...
    assert.match(stalePlan, new RegExp(`^${escapeForRegExp(uncheckedLinePrefix)}(?:\\s+—.*)?$`, "mu"));
  }
  ...
  for (const uncheckedLinePrefix of [
    ...
    assert.match(cutoverPlan, new RegExp(`^${escapeForRegExp(uncheckedLinePrefix)}(?:\\s+—.*)?$`, "mu"));
  }

  assert.match(statusAudit, /Derived only from current cutover evidence files and passing proof commands/u);
  assert.match(statusAudit, /Does not depend on inherited completion in `\.sisyphus\/plans\/opencode-context-compression-plugin\.md`/u);
  assert.match(statusAudit, /Does not depend on unchecked Final Verification Wave items F1-F4 in either plan/u);
  assert.match(statusAudit, /Deferred outside this audit: the owner-requested real-session runtime test and the full Final Wave review/u);
  assert.match(statusAudit, /Current proof basis: `cutover-9-evidence-pack\.txt`, `npm run typecheck`, and the cutover test suite listed there/u);
});
