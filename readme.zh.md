...
`DESIGN.md` 是本仓库的真相源。`DESIGN-CHANGELOG.zh.md` 只是变更提示，不得高于设计原文。
...
因此，本仓库当前生效的 reminder 资产就是下面列出的四份拆分文件。与此同时，设计里提到的 `tests/e2e/delete-route.test.ts` 在本仓库内已经收口为 `tests/e2e/allow-delete-delete-style.test.ts`，避免继续把旧 route 文件名误写成当前公开契约。
...
- `prompts/reminder-soft-compact-only.md`，当前上下文仅允许压缩时使用的软提醒文本
- `prompts/reminder-hard-compact-only.md`，当前上下文仅允许压缩时使用的硬提醒文本
- `prompts/reminder-soft-delete-allowed.md`，当前上下文允许删除时使用的软提醒文本
- `prompts/reminder-hard-delete-allowed.md`，当前上下文允许删除时使用的硬提醒文本
...
- `reminder.softRepeatEveryTokens` 与 `reminder.hardRepeatEveryTokens`：按 token 表达的 reminder 重复 cadence
- `reminder.promptPaths.compactOnly.*` 与 `reminder.promptPaths.deleteAllowed.*`：仓库自有的 reminder 文本资源路径
...
- `softRepeatEveryTokens` / `hardRepeatEveryTokens` —— 已由 deterministic reminder derivation 实际执行
...
reminder 文本现在来自四份仓库自有的纯文本资源，而不是硬编码字符串：
...
这些 reminder 资源会被显式加载，并且必须包含非空文本，但它们不是模板文件，也不要求占位符变量。真正承担 compaction 模板角色的仍然是 `prompts/compaction.md`。
...
- `mode` 取值为 `"compact"` 或 `"delete"`
- `target.startVisibleMessageID` 与 `target.endVisibleMessageID` 来自当前投射后的可见消息视图
- 该工具会先在仓库自有的投射视图里解析目标跨度，然后把 mark 与 source snapshot 一起持久化进侧车数据库

`compression_mark` 不会暴露公共执行步骤。批次冻结、调度、runner 调用与锁处理都属于插件内部运行时行为，而不是公共执行工具。

`allowDelete` 不是当前公开动作字段。当前公开动作字段是 `mode`；`allowDelete` 则是运行时当前的删除许可位，用来决定 `mode: "delete"` 是否允许进入 delete-style 路径。
...
当前对外口径要把“请求动作”“运行时许可”和“已提交结果”分开说：

- `mode="compact"` 表示请求生成 compact 型可引用 replacement
- `mode="delete"` 表示请求 delete-style 行为，但只有当前运行时许可允许时才会进入该路径
- `allowDelete=true|false` 是运行时暴露给 prompts、日志、测试和 delete-style admission 的删除许可位
- 已提交 replacement 记录的是 `executionMode="compact"` 或 `executionMode="delete"`

如果要理解更深一层的内部语义，请回到 `DESIGN.md`，不要拿 `DESIGN-CHANGELOG.zh.md` 覆盖设计原文。
...
- `tests/e2e/allow-delete-delete-style.test.ts`，用同样的 repo-owned fixture 风格验证 `allowDelete=true` 下 delete-style 行为的已提交路径

这份 README 明确不声称以下两件事：

- 当前宿主暴露的 legacy 工具已经能在真实会话里为本插件提供有效的 compact 与 delete-style 端到端证明
- 本仓库已经自带默认生产 compaction executor transport

关于当前可做的人工观察，请看 `docs/live-verification-with-mitmproxy-and-debug-log.zh.md`。那份文档保留同样的真相边界：真实会话可以确认插件加载、seam 日志、sidecar 创建等运行时副作用，但完整 `executionMode=compact` 与 `executionMode=delete` 证明目前仍以本仓库自动化测试为准。
