...
- `allowDelete` 是当前选中可见跨度的删除许可位，类型为布尔值
...
## `allowDelete` 与已提交 `executionMode`

现在持久化的 mark 与 source lineage 用 `allowDelete` 表示删除许可，而已提交 replacement 用 `executionMode` 表示这次实际产出的结果。

### `allowDelete`

- `allowDelete: false` 表示该跨度可以被压缩，但不允许进入删除分支
- `allowDelete: true` 表示该跨度后续允许进入支持删除的分支
- `allowDelete` 只记录许可，不单独表示本次批次最终实际执行了什么

### 已提交 `executionMode="compact"`

- 已提交 replacement 会作为幸存的可引用块保留在提示词可见视图里
- 原始源跨度只会在投射视图中被隐藏
- replacement 不再参与下一轮压缩

### 已提交 `executionMode="delete"`

- 压缩仍会在 SQLite 中创建已提交 replacement 记录
- 投射会把该结果渲染成极简 delete notice，而不是可复用总结块
- 一旦 delete replacement 提交，原始源跨度会从提示词可见投射中移除
- delete 结果仍通过与 compact 结果相同的 replacement 表、source snapshot 与 consumed-mark 链接追踪
- delete 输出被视为终结性清理结果，不会再次参与压缩

简而言之，`allowDelete` 记录的是许可，`executionMode` 记录的是本次已提交结果到底是 compact 幸存块还是 delete notice。
...
- `tests/e2e/delete-route.test.ts`，用同样的 repo-owned fixture 风格验证删除分支的已提交行为
