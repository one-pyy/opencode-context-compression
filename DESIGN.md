...
- `state/`、`locks/`、`logs/` 以及其他 repo-owned sidecar 路径，**都相对于插件根目录解析**
- 它们**不**相对于 OpenCode 的启动目录、当前 shell 的 working directory、或用户会话运行时所在目录解析
- 因此，无论 OpenCode 从哪里启动，只要插件根目录不变，这些 sidecar 路径就应稳定落在插件目录之下
...
`allowDelete` 是当前运行时是否允许创建 delete 型 mark 的**准入条件**，不是全局 route 枚举，也不是 mark / replacement 的长期持久语义位。

一旦 tool 调用被接受，后续历史解释主要依赖 `mode` 与结果组，而不是继续把 `allowDelete` 作为跨轮持久业务字段保存和比较。

更具体地说：

- `allowDelete` 的硬检查发生在 `compression_mark` 工具调用当下
- 若当前 `allowDelete=false` 且模型请求 `mode=delete`，该次 tool 调用应直接返回错误结果，而不是生成可重放的 delete mark
- hook / replay 在后续解释历史时，只应把**成功返回 mark id** 的 tool 结果当作 mark intent
- 若历史中的 tool 结果是错误结果，则它应保留为普通可见错误消息，但**不进入 mark 覆盖树，也不参与 replacement lookup**
...
因此，`allowDelete=true` 不是“未来再说”的能力开关，而是会影响当前轮 delete mark 的准入，以及后续 prompt / projection / replacement 渲染分支；但它本身不再作为长期持久匹配主键。
...
这里的“返回错误信息”是正式语义，不是临时实现细节：

- 错误结果表示“这次 tool 调用没有成功创建 mark”
- 因此后续 replay / hook 不得把这条调用解释成 delete mark
- 这条 tool 消息仍保留在宿主历史里，并可在最终可见世界中以普通错误消息存在
...
在当前 hook 结构下，更精确的要求是：

- 若存在活跃 compaction gate，则应在 `experimental.chat.messages.transform` 的**第一时间**执行 gate 检查
- 也就是在开始解析、重放、投影当前轮 prompt-visible 内容之前先决定是否等待
- 若 gate 仍然活跃，则在这里阻塞，直到完成、终态失败、超时或手工恢复，然后才继续当前轮 `messages.transform`
- 不允许先做部分 projection / replay，再在更晚阶段因为 gate 才中断普通对话
...
2. **SQLite 是 sidecar state，不是第二套会话**
   ...
   - 该路径相对于**插件根目录**，而不是 OpenCode 启动目录
...
```
<plugin-root>/state/<session-id>.db       — SQLite 侧车数据库
<plugin-root>/locks/<session-id>.lock     — 实时压缩锁文件
<plugin-root>/logs/runtime-events.jsonl   — 运行时事件日志
<plugin-root>/logs/seam-observation.jsonl — seam 观察日志（启用时）
```

这些路径的解析基准必须是**插件根目录**。

- 不允许把它们绑到 OpenCode 的启动目录
- 不允许把它们隐式绑到当前 shell / process working directory
- debug snapshot 路径如使用 repo-owned 相对路径，也应按同一规则相对于插件根目录解析
...
- `state/`、`locks/`、`logs/` 等 sidecar 目录都属于插件自有目录结构，应稳定落在插件根目录下
...
- **成功结果**：返回合法 `mark id`，表示这次调用成功创建了一个可重放 mark intent
- **错误结果**：表示这次调用未成功创建 mark；它仍是宿主历史中的普通 tool 消息，但后续 replay 不得把它当作 mark intent
...
- hook / replay 只对“成功返回 mark id 的 tool 结果”做 mark 语义解释
- 对于 admission 失败、参数错误、结果反序列化失败等错误 tool 调用，不得伪造 mark intent
- 这类调用应作为普通错误消息留在最终可见世界里，而不是被静默跳过
