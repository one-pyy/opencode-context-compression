...
>
> 本文档描述当前权威设计。所有实现、配置、测试与 prompt 契约都应直接以本文档正文为准，全文应保持自洽，不依赖外部迁移说明或后文章节纠偏才能成立。
...
1. 从 `opencode.jsonc` 中移除其他历史上下文压缩插件入口
...
# 检查 replacement 结果组
sqlite3 "state/<session-id>.db" "select * from result_groups;"

# 检查 replacement 片段
sqlite3 "state/<session-id>.db" "select * from result_fragments order by mark_id, fragment_index;"
...
Reminder 的重复 cadence 由显式 token 字段定义：`softRepeatEveryTokens` 与 `hardRepeatEveryTokens`。
...
`allowDelete` 是**目标消息或压缩结果是否允许被删除**的布尔许可位。
...
- 删除不是独立的第二套子系统；它仍属于同一条 mark → result group / delete-style result → projection 语义链
- 无论 `allowDelete` 取值如何，**再次压缩都被禁止**
- `allowDelete=true` 允许的“直接删除”是终结性清理，不是再次压缩

### 4.3 控制来源

`allowDelete` 是当前运行时是否允许创建 delete 型 mark 的**准入条件**。

一旦 tool 调用被接受，后续历史解释主要依赖 `mode` 与结果组，而不是继续把 `allowDelete` 作为跨轮持久业务字段保存和比较。
...
因此，`allowDelete=true` 会影响当前轮 delete mark 的准入，以及后续 prompt / projection / replacement 渲染分支；但它本身不作为长期持久匹配主键。
...
实现中的局部变量名可以使用 `hostMessageId` 这类命名来指代“当前宿主消息标识”，但其语义仍应对应 envelope `info.id`；这种命名不表示系统内存在一套独立于 `info.id` 的第二标识体系。

### 5.6 Visible ID 落库补充

作为实现补充，sidecar 应至少持久化一张“canonical host message identifier → 插件分配 visible id”的稳定映射。

该映射的目的不是保存宿主真值，而是保证：

- 同一条 canonical message 在多轮全量 replay 后仍获得同一可见编号
- 前六位序号保持严格递增（`000001`, `000002`, ...）
- 后缀仍然是基于 canonical host message identifier 计算出的稳定 base62 / checksum 风格短后缀

推荐把实现上的完整赋值 visible id 理解为：

```text
<visible-type>_<seq6>_<base62>
```

其中：

- `visible-type` 对应当前投影时的可见类别（如 `protected` / `compressible` / `referable`）
- `seq6` 是永久递增的 6 位十进制序号
- `base62` 是从 canonical host message identifier 稳定派生的短后缀

因此，sidecar 中至少要能稳定恢复：

- `canonical_id`
- `seq6`
- `base62`

并可在 projection 最终出口重新拼出当前轮需要呈现给模型的 visible id 文本。
...
当存在活跃 compaction gate 时，这个等待应发生在当前轮 `experimental.chat.messages.transform` 开始解析、投影或 materialize 当前 prompt-visible 内容之前；普通对话应在这里阻塞，直到 lock 解除、终态失败、超时或手工恢复后再继续。
...
Reminder 重复提醒语义通过显式 token 字段表达：`softRepeatEveryTokens` 与 `hardRepeatEveryTokens`。
...
2. **SQLite 是 sidecar state，不是第二套会话**
   ...
   - 实时 gate / lock 语义由文件锁和进程内状态承担，不要求额外落库运行时 job / gate 表
...
- mark/source snapshot 的真值库
- 运行时 gate / compaction job 的过程状态仓库

### 12.3A 当前最小表设计补充

为避免把 sidecar 做成“什么都存”的第二套系统，当前设计把 SQLite 的最小表规划收敛为：

1. `schema_meta`
   - 用途：记录 schema 版本与真值模型声明
   - 这是数据库自描述元数据，不是业务真值

2. `visible_sequence_allocations`
   - 用途：保存 `canonical_id -> (seq6, base62)` 的稳定映射
   - `seq6` 必须严格递增
   - 该表保证同一条消息跨多轮全量 replay 后仍得到相同 visible id

3. `result_groups`
   - 用途：保存某个 `mark id` 对应的一次**成功提交**的完整结果组头信息
   - 只保存成功结果；失败压缩不入库
   - 建议字段至少包括：
     - `mark_id`
     - `mode`
...
4. `result_fragments`
   - 用途：保存某个结果组被原始 gap 打散后的有序 replacement 片段
   - 建议字段至少包括：
     - `mark_id`
     - `fragment_index`（从 `0` 开始）
     - `source_start_seq`
     - `source_end_seq`
     - `replacement_text`
...
- 不单独持久化 `marks` 真值表
- 不单独持久化 `source_snapshots` 真值表
- 不持久化 `fragment_kind`；fragment 的语义就是“同一结果组被 gap 打散后的有序 replacement 片段”

gap 本身不入库；projection 在全量 replay 原始历史时自然把 gap 补回。
...
- `sequence-service` — 维护 canonical host message identifier 到稳定 visible-id（`seq6 + base62`）的映射，并在渲染出口生成最终 visible id 文本
- `mark-service` — 从历史重放 mark tool 调用，构造当前有效覆盖树；不持久化独立 mark 真值表
- `replacement-service` — 按 mark id 读取 replacement 结果组并校验其是否完整；不依赖 canonical source 持久表做主 lookup
...
- `compaction-input-builder` — 从当前合法 mark 所覆盖的原始历史范围构造压缩专用输入；不要复用 projected prompt view
...
- `tests/cutover/runtime-seam-independence.test.ts` — runtime / tool / provider seam 的规范执行
...
- `tests/e2e/delete-path.test.ts` — `allowDelete=true` / delete-style 行为覆盖
...
### 14.5 `allowDelete` 定义 delete mark 的准入策略

`allowDelete` 决定当前运行时是否允许创建 delete 型 mark。
...
### 14.22 Reminder cadence 使用显式 token 字段

Reminder 重复提醒由显式 token 字段表达：`softRepeatEveryTokens` 与 `hardRepeatEveryTokens`。
...
运行时实现应显式维护与当前有效覆盖树一致的 token 统计口径，并据此计算“未压原始 token”是否达到自动压缩阈值；不能用 mark 数量、树节点数量或其他简化计数代替这一 readiness 判断。

---

## 15. 2026-04 运行时重放模型详细解说

> 本章是对当前运行时模型的详细展开：把前文已经给出的规则，用更完整的生命周期说明、形式化约束与大量例子重新讲透。

### 15.1 运行时模型总述：模型选择目标，执行在后台异步完成
...
1. **mark 的真相源是宿主历史中的 tool 调用记录**
   - `compression_mark` 调用会把输入参数与返回的 `mark id` 留在宿主历史中
   - projection / hook 通过顺序重放这些历史记录来恢复当前有效 mark 结构

2. **`allowDelete` 只定义 delete mark 的当前准入策略**
   - `allowDelete` 决定当前运行时是否允许创建 `mode=delete` 的 mark
   - 一旦 tool 调用已被接受，后续历史解释、replacement lookup 与 projection 渲染主要依赖 `mode` 与结果组，而不依赖跨轮持久化的 `allowDelete` 业务位

3. **compact 结果不能被内部再次改写，但可以作为更大范围中的原子片段继续参与后续结构**
   - compact 结果可以进入更大的后续 compact 范围，作为不可压缩原子块处理
   - compact 结果也可以被后续合法的 delete 整段覆盖

4. **replacement 命中以历史里真实出现的 `mark id` 为主键**
   - hook 先按历史重放构造有效覆盖树
   - 再按 `mark id` 查询数据库中是否存在完整结果组
   - 找到完整结果组才替换；否则回退到子节点结果或保留原位置内容
...
当前模型下，mark tool 调用不在调用时写入独立的持久化 mark 真值表。当前权威语义是：
...
### 15.5 hook 的总体职责：模拟执行并重建当前有效 mark 结构
...
当前设计收敛到“每次 hook 都从头到尾 replay 当前历史”，原因不是为了追求概念洁癖，而是因为这能最稳定地满足以下三个目标：

1. **Reminder 插入位置必须由整段历史共同决定**
   - `hsoft` / `hhard` 的 token 口径依赖整段当前可见历史
   - 某个窗口压缩成功后，旧 reminder 还可能消失或换锚点
   - 因此 reminder 位置不能只靠增量 checkpoint 局部推断

2. **mark 覆盖树必须按完整历史顺序重建**
   - later cover earlier
   - 相交报错
   - fallback 到子节点
   - 这些都要求按完整顺序重放 mark tool 调用

3. **visible id 与最终投影都要求确定性**
   - 相同 canonical history + 相同结果组 → 相同最终投影
   - 每轮全量 replay 更容易保证这一点

因此，当前补充设计明确：

- sidecar 不要求保存 replay checkpoint
- correctness 以“从头到尾全量 replay + sidecar 结果组 lookup”为准
- 若未来引入增量优化，也必须建立在这一 correctness 模型之上，而不能替代它
...
### 15.12A 结果组落库与 fragment 语义补充

当前实现补充把结果组进一步收敛为：

- `result_groups` 只保存**成功压缩/删除后已经提交**的整体结果
- “整组完整成功才算有效”是 **insert 事务规则**，不是额外的复杂状态机
- 因此失败压缩不应写入一条 `failed result group` 供 projection 跳过；它应当根本不入结果组读模型

与之对应，`result_fragments` 的语义也应保持简单：

- fragment 就是“同一结果组被原始 gap 打散后的有序 replacement 片段”
- `fragment_index` 从 `0` 开始递增
- 不再单独持久化 `fragment_kind`
- gap 本身不存数据库；projection 按原始历史和 fragment 覆盖范围自然补回 gap

因此，一个 mark 的结果组落库规则是：

```text
要么 result_groups + 全部 result_fragments 一次性提交成功
要么整组都不存在
```

这只是把 §15.12 的“原子生效”要求进一步落实到表设计与事务语义上，而不是引入新的业务含义。
...
运行时应围绕当前有效覆盖树维护这一指标对应的显式数据结构；至少需要能表示每个有效节点的原始 token、已压 token 与未压 token，并支持沿树向上聚合。按 mark 数量或其他简化计数做 readiness 判断都不满足这一口径。
...
为保持本文全文自洽，以下语义应在所有实现与审计中保持一致：

1. `allowDelete` 只表示 delete mark 的当前准入策略，不作为 mark / replacement 的长期持久业务字段
2. projection 的主入口是历史重放 + 结果组 lookup，而不是独立持久化 mark/source snapshot 真值表
3. “压缩后内容不能再次压缩”指的是不能再次内部改写 compact 结果；不表示该结果块不能进入更大范围，也不表示它不能被 delete 覆盖
4. 合法但尚无结果的 mark 仍保留在覆盖树中；模拟执行阶段已判错的 tool 调用只显示为普通错误消息，不再进入 mark 语义系统
