...
[decisions/2026-07-03_async-compaction-with-decoupled-replacement-gate.md] — 压缩 N+1 启动并解耦替换门槛；每次发送只跑一轮完整模型链，跨发送失败上限配置化且默认 99999 #compaction #architecture #runtime #scheduling #async #replacement-gate #retry #failure-handling
...
[learnings/2026-06-28_child-agent-task-sessions-are-plugin-visible.md] — `task(session_id=...)` 子 agent 会话会进入插件 seam，问答式执行需由主 agent 续轮编排 #subagent #task-session #runtime #projection #seams #pattern
