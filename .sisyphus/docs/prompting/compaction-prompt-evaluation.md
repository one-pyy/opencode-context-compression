...
- **指针类**（路径、行号、函数名、错误码、命令、URL）：普通指针可以短引用；源契约文件、已批准计划、恢复点或 checkpoint、外化产物、日志/快照和未来 AI 必须重访的配置或文档路径属于关键路径，首次出现在 `<compression_output>` 时必须保留完整形式，后续才可短写；`<plan>`、`<explanation>` 和 opaque 内容不计入首次出现
