...
```text
{time}-{sessionID}-{markStartSeq}-{markEndSeq}-{model}-attempt{n}.in.json
{time}-{sessionID}-{markStartSeq}-{markEndSeq}-{model}-attempt{n}.out.json
```

要求：

- `time` 放在最前面，使用文件名安全的 UTC 时间戳；同一次请求的 `.in.json` 与 `.out.json` 使用同一个 time 前缀。
...
- `.in.json` 保存实际传给 transport 的压缩模型请求。
- `.out.json` 只保存 transport 返回的原始模型 payload。
- 如果 transport 在拿到 payload 前抛错，可能只有 `.in.json`，不会为了表达失败而写入 `.out.json`。
