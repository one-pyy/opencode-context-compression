...
- 二者共用同一套“范围 → 模型 → result group → projection 替换”机制。
- delete 的输入选择、有效信息保留与实施状态以 [删除契约](allow-delete.md#实施状态) 为准；输入构造需消费该模式的有效表示，不能统一展开为全部原始历史。
- 覆盖树与整组提交原则继续适用。新 delete 失败时，已存在的子结果仍按渲染算法提供 fallback，不因父范围已标记而丢弃。
- 调度与替换门槛遵守本文对应章节；delete 的模型输入预算遵守 [输入选择](allow-delete.md#输入选择)，两者独立判断。
